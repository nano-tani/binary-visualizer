...
C.boot=()=>{C.updateHome();C.initModes();C.initQuiz();C.initProgress();C.initCopy();C.initUtility();C.initGlossary();C.initLabTracking();C.initPrivacy();C.initAnalytics();if(C.initLab)C.initLab()};
const start=()=>C.boot();
